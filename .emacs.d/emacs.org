...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-ytdl-url (&optional url)
    "Run 'youtube-dl' over the URL.  If URL is nil, use URL at point."
    (interactive)
    (let* ((url (or url (thing-at-point-url-at-point)))
           (new-buffer
            (apply 'make-comint-in-buffer "youtube-dl" nil
                   "ytdl" nil (list url)))
           (ytdl-proc (get-buffer-process new-buffer)))
      (set-process-sentinel ytdl-proc 'my-kill-buffer-sentinel)
